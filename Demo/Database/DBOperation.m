...
+(void)checkCreateDB{
    @try {
        ...
        NSString *imageFolderName,*imageFolderPath;
        
        databaseName=@"SIMPL.sqlite";
        ...
        //userd for imageFolderCopy
        
         imageFolderPath = [docDir stringByAppendingString:imageFolderName];
        
        BOOL FolderSuccess;
       
        FolderSuccess = [fm fileExistsAtPath:imageFolderPath];
        if (FolderSuccess)
        {
            return;
        }
        
        NSString *folderPathFromApp = [[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Images" ];
//        NSString *folderPathFromApp = [[NSBundle mainBundle] pathForResource:@"Images" ofType:nil];
        [fm copyItemAtPath:folderPathFromApp toPath:imageFolderPath error:nil];
